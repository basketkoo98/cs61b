 ...
 *  Clients create squares using the factory method sq, not the
 *  constructor.  Because there is a unique Square object for each
...
final class Square {
    ...
    /** Return the distance from me to SQ. */
    int distance(Square sq) {
        return Math.abs(_col - sq.col()) + Math.abs(_row - sq.row());
    }


    /** Return the distance from mySquare to the closest edge. */
    int distanceToEdge() {
        int distance;
        if (_row < BOARD_SIZE) {
            distance = _row;
        ...
        if (_col < BOARD_SIZE) {
            distance = Math.min(distance, _col);
        } else {
            distance = Math.min(distance, 8 - _col);
        }
        return distance;
    }
    ...
    /** Return the diagonally adjacent square of SQ. */
    static HashSet<Square> diagonal(Square sq) {
        HashSet<Square> result = new HashSet<>();
        if (exists(sq.col() + 1, sq.row() + 1)) {
            result.add(sq(sq.col() + 1, sq.row() + 1));
        }
        if (exists(sq.col() - 1, sq.row() + 1)) {
            result.add(sq(sq.col() - 1, sq.row() + 1));
        }
