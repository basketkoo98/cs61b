...
class BoardWidget extends Pad {
    ...
    /** Column index. */
    static final String COL_INDEX = "abcdefghi";
    ...
    private void drawGrid(Graphics2D g) {
        ...
        if (_acceptingMoves && _hasClicked) {
            g.setColor(CLICKED_SQUARE_COLOR);
            g.fillRect(cx(_clicked), cy(_clicked),
                    SQUARE_SIDE, SQUARE_SIDE);
        }
        ...
        for (int i = 1; i <= 9; i++) {
            ...
            g.drawString(Integer.toString(i),
                    cx(sq(0, i - 1)) + 2 * OFFSET - MARGIN,
                    cy(sq(0, i - 1)) + SQUARE_SIDE / 2 + 2 * OFFSET);
            g.drawString(COL_INDEX.substring(i - 1, i),
                    cx(sq(i - 1, 0)) + SQUARE_SIDE / 2,
                    cy(sq(i - 1, 0)) +  3 * SQUARE_SIDE / 2);
        }
    ...
    private void drawPiece(Graphics2D g, Square s) {
        if (_board.get(s) == Piece.BLACK) {
            ...
            g.drawOval(cx(s) + OFFSET,
                    cy(s) + OFFSET, SQUARE_SIDE - 2 * OFFSET,
                    SQUARE_SIDE - 2 * OFFSET);
            g.setColor(BLACK_COLOR);
            g.fillOval(cx(s) + OFFSET,
                    cy(s) + OFFSET, SQUARE_SIDE - 2 * OFFSET,
                    SQUARE_SIDE - 2 * OFFSET);
        }
    ...
    private void click(Square s) {
        if (_hasClicked) {
            ...
            if ((_board.get(s) == Piece.BLACK && _board.turn() == Piece.BLACK)
                    || ((_board.get(s) == Piece.KING
                    || _board.get(s) == Piece.WHITE)
                    && _board.turn() == Piece.WHITE)) {
                _hasClicked = true;
                _clicked = s;
            }
        }
    ...
    /** Return true if mouse already clicked a square. */
