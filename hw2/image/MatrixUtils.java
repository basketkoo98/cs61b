...
public class MatrixUtils {
    ...
    public static double[][] accumulateVertical(double[][] m) {
        ...
        for (int i = 1;i<rowLength;i++){
            result[i][0]= Math.min(result[i - 1][0]+m[i][0], result[i - 1][1]+m[i][0]);
            result[i][colLength-1]= Math.min(result[i - 1][colLength-1]+m[i][colLength-1], result[i - 1][colLength-2]+m[i][colLength-1]);
            for (int j = 1;j<colLength-1;j++){
                result[i][j]= Math.min(result[i - 1][j-1]+m[i][j],result[i - 1][j]+m[i][j]);
                result[i][j] = Math.min(result[i][j], result[i - 1][j+1]+m[i][j]);
            }
        }
    ...
    public static double[][] accumulate(double[][] m, Orientation orientation) {
        if (orientation == Orientation.VERTICAL) {
            ...
            double[][] resultT = accumulateVertical(mT);
            double[][] result = new double[m.length][m[0].length];
            for (int i = 0; i < m.length; i++) {
                for (int j = 0; j < m[0].length; j++) {
                    result[i][j] = resultT[j][i];
                }
            }
            return result; //your code here
        }
