    ...
    private int makeForwardConnections(Model model) {
        ...
        for (Sq sq : model) {
            ...
            if (sq.successor() == null && sq.direction() != 0) {
                for (Place successor: sq.successors()) {
                    if (sq.connectable(model.get(successor))) {
                        nFound += 1;
                        found = model.get(successor);
                        if (sq.sequenceNum() + 1 == found.sequenceNum()
                                && sq.sequenceNum() != 0
                                && found.sequenceNum() != 0) {
                            nFound = 1;
                            break;
    ...
    private int makeBackwardConnections(Model model) {
        ...
        for (Sq sq : model) {
            ...
            if (sq.predecessor() == null && sq.sequenceNum() != 1) {
                for (Place predecessor : sq.predecessors()) {
                    if (model.get(predecessor).connectable(sq)) {
                        nFound += 1;
                        found = model.get(predecessor);
                        if (found.sequenceNum() + 1
                                == sq.sequenceNum() && found.sequenceNum()
                                != 0 && sq.sequenceNum() != 0) {
                            nFound = 1;
                            break;
                        }
