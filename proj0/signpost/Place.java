...
class Place {
    ...
    static int dirOf(int x0, int y0, int x1, int y1) {
        ...
        int result = dx > 0 ? 2 - dy : dx == 0 ? 6 + 2 * dy : 6 + dy;
        return result;
    }
        ...
        for (int i = 0; i < width; i++) {
            for (int j = 0; j < height; j++) {
                ...
                for (int k = 1; k <= 8; k++) {
                    ...
                    int colIndex = i;
                    int rowIndex = j;
                    while (0 <= colIndex && colIndex < width
                            && 0 <= rowIndex && rowIndex < height) {
                        colIndex += DX[k];
                        rowIndex += DY[k];
                        if (0 <= colIndex && colIndex < width
                                && 0 <= rowIndex && rowIndex < height) {
                            M[i][j][k].add(_places[colIndex][rowIndex]);
                        }
                    }
                    for (Place pl : M[i][j][k]) {
                        M[i][j][0].add(pl);
                    }
                }
            }
