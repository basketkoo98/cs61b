...
class Model implements Iterable<Model.Sq> {
    ...
    Model(int[][] solution) {
        ...
        _solutionIndext = new int[_width * _height];
        for (int col = 0; col < _width; col++) {
            for (int row = 0; row < _height; row++) {
                _solutionIndext[solution[col][row] - 1] = _width >= _height
                                ? col * _width + row : col + row * _height;
            }
        }
        ...
        for (int col = 0; col < _width; col++) {
            for (int row = 0; row < _height; row++) {
                if (solution != null) {
                    _solnNumToPlace[solution[col][row] - 1]
                            = Place.pl(col, row);
                } else {
                    throw badArgs("All numbers from 1 - last don't appear.");
                }
            }
        }
        ...
        for (Sq[] col: _board) {
            for (Sq sq : col) {
                ...
                for (Sq possible: _allSquares) {
                    if (possible._sequenceNum != last) {
                        if (Place.dirOf(possible.x, possible.y, sq.x, sq.y)
                                == possible._dir) {
                            sq._predecessors.add(possible.pl);
                        }
                    }
                }
            }
    ...
    Model(Model model) {
        ...
        for (Sq[] col : this._board) {
            for (Sq sq : col) {
                this._allSquares.add(sq);
            }
        }

        for (Sq[] col : this._board) {
            for (Sq sq : col) {
                Sq successor = model._board[sq.x][sq.y].successor();
                Sq predecessor = model._board[sq.x][sq.y].predecessor();
                Sq head = model._board[sq.x][sq.y].head();
                if (successor == null) {
                    sq._successor = null;
                } else {
                    sq._successor = this.get(successor.x, successor.y);
                }
                if (predecessor == null) {
                    sq._predecessor = null;
                } else {
                    sq._predecessor = this.get(predecessor.x, predecessor.y);
                }
    ...
    void restart() {
        ...
        for (Sq sq : this) {
            if (sq._successor == null && sq._predecessor == null) {
                sq._group = -1;
            }
        }
        ...
        for (int i = 0; i < _allSquares.size() - 1; i++) {
            if (_allSquares.get(i)._sequenceNum != 0) {
                for (int j = i + 1; j < _allSquares.size(); j++) {
                    if (_allSquares.get(j)._sequenceNum != 0) {
                        if (_allSquares.get(i).connect(_allSquares.get(j))
                                || _allSquares.get(j).connect(
                                        _allSquares.get(i))) {
                            makeChanges = true;
                        }
                    }
    ...
    private int arrowDirection(int x, int y) {
        ...
        if (seq0 < _width * _height) {
            int x1 = _width >= _height ? _solutionIndext[seq0] / _width
                    : _solutionIndext[seq0] % _height;
            int y1 = _width >= _height ? _solutionIndext[seq0] % _width
                    : _solutionIndext[seq0] / _height;
            int direction = Place.dirOf(x, y, x1, y1);
            return Math.max(0, direction);
        } else {
            return 0;
        }
    }
    ...
    final class Sq {
         ...
         *  nothing. */
        void unfixNum() {
            Sq next = _successor, last = _predecessor;
            _hasFixedNum = false;
            disconnect();
            if (last != null) {
                last.disconnect();
        ...
        boolean connectable(Sq s1) {
            if (_sequenceNum != 0 && s1.sequenceNum() != 0) {
                return pl.dirOf(s1.pl) == _dir && s1.predecessor() == null
                        && s1._sequenceNum != 1 && _successor == null
                        && _sequenceNum == s1.sequenceNum() - 1;
            } else if (_sequenceNum == 0 && s1.sequenceNum() == 0) {
                return pl.dirOf(s1.pl) == _dir && s1.predecessor() == null
                        && _successor == null
                        && (_group != s1.group() || _group == -1);
            } else {
                return pl.dirOf(s1.pl) == _dir && s1.predecessor() == null
                        && s1._sequenceNum != 1 && _successor == null;
            }
        ...
        boolean connect(Sq s1) {
            ...
            if (_sequenceNum != 0 && s1._sequenceNum == 0) {
                Sq nextSuccessor = _successor;
                int seq = _sequenceNum + 1;
                int groupToRelease = s1.group();
                while (nextSuccessor != null) {
                    nextSuccessor._sequenceNum = seq;
                    seq++;
                    nextSuccessor._group = _group;
                    nextSuccessor = nextSuccessor._successor;
                }
                releaseGroup(groupToRelease);
            } else if (_sequenceNum == 0 && s1._sequenceNum != 0) {
                Sq lastPredecessor = s1._predecessor;
                int seq = s1._sequenceNum - 1;
                int gruopToRelease = _group;
                while (lastPredecessor != null) {
                    lastPredecessor._sequenceNum = seq;
                    lastPredecessor._group = s1._group;
                    seq--;
                    lastPredecessor = lastPredecessor._predecessor;
                }
                releaseGroup(gruopToRelease);
            } else if (_sequenceNum == 0 && s1._sequenceNum == 0) {
                _head._group = joinGroups(_group, s1._group);
            } else {
                _head._group = 0;
            }
            Sq next = _head._successor;
            while (next != null) {
                next._head = _head;
                next._group = _head._group;
         ...
         * set group to -1).
         */
        void groupAndNumPredecessor() {
            boolean predecessorsAllNotFixed = true;
            if (!hasFixedNum()) {
                Sq lastPredecessor = _predecessor;
                while (lastPredecessor != null) {
                    if (lastPredecessor.hasFixedNum()) {
                        predecessorsAllNotFixed = false;
                    }
                    lastPredecessor = lastPredecessor._predecessor;
                }
            } else {
            ...
            if (predecessorsAllNotFixed) {
                ...
                if (lastPredecessor != null) {
                    ...
                    _group = group;
                    while (lastPredecessor != null) {
                        lastPredecessor._sequenceNum = 0;
                        lastPredecessor._group = group;
                        lastPredecessor = lastPredecessor._predecessor;
                    }
                } else {
                    _group = -1;
        ...
        void disconnect() {
            ...
            if (_sequenceNum == 0) {
                if (_predecessor == null && next._successor == null) {
                    ...
                    Sq nextSuccessor = next._successor;
                    while (nextSuccessor != null) {
                        nextSuccessor._group = next._group;
                        nextSuccessor = nextSuccessor._successor;
                    }
                }
            } else {
                groupAndNumPredecessor();
                boolean successorsAllNotFixed = true;
                if (!next.hasFixedNum()) {
                    Sq nextSuccessor = next;
                    while (nextSuccessor != null) {
                        if (nextSuccessor.hasFixedNum()) {
                            successorsAllNotFixed = false;
                        }
                        nextSuccessor = nextSuccessor._successor;
                    }
                } else {
                ...
                if (successorsAllNotFixed) {
                    ...
                    if (nextSuccessor != null) {
                        ...
                        next._group = group;
                        while (nextSuccessor != null) {
                            nextSuccessor._sequenceNum = 0;
                            nextSuccessor._group = group;
                            nextSuccessor = nextSuccessor._successor;
                        }
                    } else {
                        next._group = -1;
    ...
    /** The vector store the index in sequence number. For example,
     * if solution[2][3] = 5,solutionIndex[4] = 2 * _width + 3 */
    private int[] _solutionIndext;
