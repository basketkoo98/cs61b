...
public class CompoundInterest {
    ...
    static double futureValue(double presentValue, double rate,
                              int targetYear) {
        return presentValue * power(1 + rate * 0.01, numYears(targetYear)) ;
    }
    ...
    static double futureValueReal(double presentValue, double rate,
        ...
        return futureValue(presentValue,rate,targetYear) * power(1-0.01*inflationRate,numYears(targetYear)) ;
    }

    static double power(double base, int index){
        double result = 1;
    ...
    static double totalSavings(double perYear, int targetYear, double rate) {
        ...
        for (int i = THIS_YEAR; i <= targetYear; i++){
            result += futureValue(perYear,rate,i);
        }
        return result;
    }
    ...
    static double totalSavingsReal(double perYear, int targetYear, double rate,
        ...
        double result = 0;
        for (int i = THIS_YEAR; i <= targetYear; i++){
            result += futureValueReal(perYear,rate,i,inflationRate);
        }
        return  result;
    }
